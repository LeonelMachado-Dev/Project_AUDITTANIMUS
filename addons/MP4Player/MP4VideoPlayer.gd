...
# clear_buffer() crashes while the engine audio generator is active.
# Only clear when playback is not running to avoid the "Condition active" error.
func _clear_audio_buffer_if_safe() -> void:
	if audio_playback == null:
		return
	if audio_player != null and audio_player.is_playing():
		return
